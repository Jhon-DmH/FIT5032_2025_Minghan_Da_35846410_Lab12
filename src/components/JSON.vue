...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>Click an author to highlight:</p>
      <ul>
        <li v-for="author in authors" :key="author.id"
          @click="selectedAuthorId = author.id"
          :class="{ highlight: author.id === selectedAuthorId }"
          :style="{ cursor: 'pointer', fontWeight: author.id === selectedAuthorId ? 'bold' : 'normal' }">
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
    </section>
...
const orwell = computed(() => {
  // Find the author object where name === "George Orwell"
...
const austen = computed(() => {
  // Find the author object where id === 1 (Jane Austen)
